...
const toolsRegistry = {
    ...
    searchKnowledgeBase: async (args, user) => {
        if (!hasToolPermission(user.role, 'searchKnowledgeBase')) {
            return createPermissionDeniedError(user.username, user.role, 'searchKnowledgeBase');
        }
        ...
        try {
            const answer = await RAGModule.query(query);
            return {
                status: 'success',
                query,
                result: answer,
            ...
            return { error: `Failed to search knowledge base: ${err.message}` };
        }
    },
};
...
const tools = [
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Search internal documents, company policies, and vector knowledge base for accurate context.',
            parameters: {
                type: 'object',
                properties: {
                    query: {
                        type: 'string',
                        description: 'The query or subject to search for in the knowledge base.',
                    },
                },
                required: ['query'],
            },
